...
describe('user controller tests', () => {
  ...
  it('GET by id with roles returns 200 and specific item', async () => {
    const response = await request.get(url + "/roles/" + TestData.ID).set("Authorization", await getAdminToken());
    expect(response.status).toBe(200);
    expect(response.body).toEqual(TestData.TEST_USER_OUTPUT_DTO)
  })
  ...
  it('PUT returns 200 and updated item', async () => {
    ...
    const gewijzigdeData = { id: "updateUser", name: "gewijzigdeNaam", roles: ["ADMIN"] }
  ...
  it('GET by id with roles returns 400 with bad credentials', async () => {
    const response = await request.get(url + "/roles/" + TestData.ID);
    expect(response.status).toBe(400);
  })
